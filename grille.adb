package body Grille is
 	...
 	--Verifie si la cellule de coordonnees (X,Y) de la grille est vide
   function IsEmpty (Un_Plateau : grille;  X : colonne; Y : ligne)  return Boolean is
   begin
      return Cellules.IsEmpty (Un_Plateau.Un_Contenu (Y, X));
   end IsEmpty;

 	--Verifie si la cellule de coordonnees (X,Y) de la grille n'est pas vide
   ...
   begin
 	...
 	--Verifie si la cellule de coordonnees (X,Y) de la grille est occupee par le symbole 'Un_symbole'
   ...
   begin
      return (not Cellules.IsEmpty(Un_Plateau.Un_Contenu (Y, X))) and then (Cellules.IsNotEmpty(Un_Plateau.Un_Contenu (Y, X))= Un_Symbole);
   	...
   	----Essaye de placer le symbole 'Un_symbole' dans la cellule de coordonees (Y,X) de la grille
   ...
   begin
 	...
 	--Verifie si la grille est pleine c'est a  dire si les 9 coups ont etes joues
